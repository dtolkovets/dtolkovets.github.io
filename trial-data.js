  ...
  pages: {
    ...
    'smart-glasses': {
      ...
      media:[
        ...
        HEAD('Software workflow',"UML activity diagram of the glasses' interaction loop. Labels in German."),
        { ...CONTAIN('assets/smart-glasses/software-workflow.svg','Button-controlled image capture and audio recording, followed by transcription, a multimodal response and speech playback.'), fullSize:true, flush:true, alt:'Activity diagram showing initialisation, button-controlled image capture and audio recording, transcription, response generation, speech playback and shutdown.' }
      ]
    },
    'cobot': {
      ...
      media:[
        ...
        { ...CONTAIN('assets/voice-cobot/uml-manager-client.svg','Manager (client): the central controller. It requests the Speech, Vision and Arm services in turn and steers the flow with two variables, next_srv (which service runs next) and target (which tool is meant).'), fullSize:true, flush:true, alt:'Activity diagram of the manager client: after initialising the client-service communication it sets next_srv to Speech, then calls the Speech, Vision or Arm service depending on next_srv and updates target and next_srv after each call.' },
        { ...CONTAIN('assets/voice-cobot/uml-speech-service.svg','Speech service: a button press starts and ends the recording, Whisper transcribes it, and GPT-4o matches the command against the known actions to set next_srv and target. If no action fits, audio feedback asks for a new command.'), fullSize:true, flush:true, alt:'Activity diagram of the speech service: audio feedback signals readiness, a button press starts and a second one ends the recording, the audio is transcribed with the Whisper API and analysed with the GPT-4o API, the command is matched against fetching and stowing the hammer or the screwdriver, next_srv and target are set accordingly, and audio feedback announces the next action.' },
        { ...CONTAIN('assets/voice-cobot/uml-vision-service.svg','Vision service: finds the requested tool with a custom-trained YOLO11m-OBB model and returns its spatial coordinates. If the tool is not in the collaboration space, the user gets audio feedback and a new voice command is requested.'), fullSize:true, flush:true, alt:'Activity diagram of the vision service: object detection with YOLO-OBB, a check whether the detected class matches target, depth measurement at the object centre and conversion of pixel to spatial coordinates, otherwise audio feedback that the object was not found and a return to the speech service.' },
        { ...CONTAIN('assets/voice-cobot/uml-arm-service.svg','Arm service: two routines. Stowing grasps the tool at its detected position and places it in its storage slot; fetching takes the tool from its fixed slot and places it in the collaboration space. Both end in the rest position and report completion to the manager.'), fullSize:true, flush:true, alt:'Activity diagram of the arm service: a decision between fetching and stowing, the matching grasp and release routine for each, then the arm returns to its rest position and next_srv is set to Speech.' }
      ]
    },
    'rag-chatbot': {
